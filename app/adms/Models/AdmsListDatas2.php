...
class AdmsListDatas2
{
    ...
    public function listDatas2(int $page = null): void
    {
        $this->page = (int) $page ? $page : 1;

        $pagination = new \App\adms\Models\helper\AdmsPagination(URLADM . 'list-datas2/index');
        $pagination->condition($this->page, $this->limitResult);
        $pagination->pagination("SELECT COUNT(id) AS num_result FROM sequencia");
        $this->resultPg = $pagination->getResult();

        $listUsers = new \App\adms\Models\helper\AdmsRead();
        $listUsers->fullRead("SELECT id,id_seq,prim_col,segun_col,tercer_col,quarta_col,quinta_col,sexta_col,set_col,oit_col,none_col,dec_col,decPri_col,decSeg_col,decTec_col,decQuar_col,decQuit_col,creatdat
                    FROM repetir
                    ORDER BY id DESC
        ...
        if ($this->resultBd) {
         //   $_SESSION['msg'] = "<p style='color: green'>Os ultimos 40 registros!</p>";
            $this->result = true;
    ...
    public function searchData(): void
    {
       ...
       $this->resultPg = $pagination->getResult();
        ...
        $listUsers = new \App\adms\Models\helper\AdmsRead();
        $listUsers->fullRead(
                   "SELECT id,id_seq,prim_col,segun_col,tercer_col,quarta_col,quinta_col,sexta_col,set_col,oit_col,none_col,dec_col,decPri_col,decSeg_col,decTec_col,decQuar_col,decQuit_col,creatdat
                    FROM repetir
                    WHERE creatdat BETWEEN  :date_in AND :date_fn
                    ORDER BY id DESC
